...
async def _refuse_to_serve_unauthenticated() -> None:
    ...
    if _open_mode_requested():
        ...
        return
    # Present-but-empty is its own failure and needs its own words. On 16 Sep 2026 four
    # revisions refused to start with "is not set" while the variable was plainly listed
    # on each of them: a key generated by `$(openssl rand -hex 32)` in PowerShell, where
    # openssl was not on the path, had come out as an empty string, and gcloud accepted
    # "ONTOLEAP_API_KEY=" without complaint.
    if "ONTOLEAP_API_KEY" in os.environ:
        problem = ("ONTOLEAP_API_KEY is set but empty, so this service will not start. "
                   "Whatever generated the key produced nothing - check it on its own "
                   "before passing it to the deploy")
    else:
        problem = "ONTOLEAP_API_KEY is not set, so this service will not start"
    raise RuntimeError(
        problem + ". Without a key every endpoint is reachable without credentials and "
        "anyone who can reach the service can spend metered Gemini / Google Knowledge "
        "Graph quota. Generate one with "
